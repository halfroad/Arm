...
//升级说明 
//20190512
//1,MPU_Set_Protection函数添加de参数,用于控制禁止/允许指令访问选项
//////////////////////////////////////////////////////////////////////////////////	 
...
{
	MPU_Set_Protection(0x20000000,128*1024,1,0,MPU_REGION_FULL_ACCESS,0,1,1);		//保护整个DTCM,共128K字节,允许指令访问,禁止共用,允许cache,允许缓冲
	
	MPU_Set_Protection(0x24000000,512*1024,2,0,MPU_REGION_FULL_ACCESS,0,1,1);		//保护整个AXI SRAM,共512K字节,允许指令访问,禁止共用,允许cache,允许缓冲
	MPU_Set_Protection(0x30000000,512*1024,3,0,MPU_REGION_FULL_ACCESS,0,1,1);		//保护整个SRAM1~SRAM3,共288K字节,允许指令访问,禁止共用,允许cache,允许缓冲
	MPU_Set_Protection(0x38000000,64*1024,4,0,MPU_REGION_FULL_ACCESS,0,1,1);		//保护整个SRAM4,共64K字节,允许指令访问,禁止共用,允许cache,允许缓冲

	MPU_Set_Protection(0x60000000,64*1024*1024,5,0,MPU_REGION_FULL_ACCESS,0,0,0);	//保护MCU LCD屏所在的FMC区域,,共64M字节,允许指令访问,禁止共用,禁止cache,禁止缓冲
	MPU_Set_Protection(0XC0000000,64*1024*1024,6,0,MPU_REGION_FULL_ACCESS,0,1,1);	//保护SDRAM区域,共64M字节,允许指令访问,禁止共用,允许cache,允许缓冲
	MPU_Set_Protection(0X80000000,256*1024*1024,7,1,MPU_REGION_FULL_ACCESS,0,0,0);	//保护整个NAND FLASH区域,共256M字节,禁止指令访问,禁止共用,禁止cache,禁止缓冲
}
