...
{	
	GPIO_Set(GPIOA,PIN5,GPIO_MODE_OUT,GPIO_OTYPE_PP,GPIO_SPEED_MID,GPIO_PUPD_PD);//PA5推挽输出
	GPIO_Pin_Set(GPIOA,PIN5,0);//PA5输出0,放电
	delay_ms(5);
	TIM2->SR=0;   	//清除标记
	TIM2->CNT=0;	//归零     
	GPIO_Set(GPIOA,PIN5,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_MID,GPIO_PUPD_NONE);//PA5,复用功能,不带上下拉 
}
//得到定时器捕获值
//如果超时,则直接返回定时器的计数值.
...
//定时器2通道2输入捕获配置	
//TIM2的时钟来自APB1,当D2PPRE1≥2分频的时候
//TIM2的时钟为APB1时钟的2倍，而APB1为100M
//所以,TIM2的时钟频率为200Mhz
//arr：自动重装值
//psc：时钟预分频数
