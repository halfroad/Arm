...
TIM_HandleTypeDef g_atimx2_handle;              /* 定时器x句柄 */
TIM_OC_InitTypeDef g_atimx_oc_chy_handle;       /* 定时器输出句柄 */
TIM_OC_InitTypeDef g_atimx2_oc_chy_handle;      /* 定时器输出句柄 */
...
void HAL_TIM_PWM_MspInit(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == ATIM_TIMX_PWM)
    {
        ...
        HAL_NVIC_SetPriority(ATIM_TIMX_PWM_IRQn, 2, 2);
    ...
    {
        GPIO_InitTypeDef gpio_init_struct;
        ATIM_TIMX2_PWM_CHY_CLK_ENABLE();                            /* 通道y的GPIO口 */
        ...
        /*定时器IO初始化*/
        gpio_init_struct.Pin = ATIM_TIMX2_PWM_CH1_GPIO_PIN;         /* 通道y的GPIO口 */
        gpio_init_struct.Mode = GPIO_MODE_AF_PP;                    /* 复用推挽输出 */
        gpio_init_struct.Pull = GPIO_NOPULL;                        /* 上拉 */
        gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;              /* 高速 */
        ...
        gpio_init_struct.Pin = ATIM_TIMX2_PWM_CH2_GPIO_PIN;         /* 通道y的GPIO口 */
        HAL_GPIO_Init(ATIM_TIMX2_PWM_CH2_GPIO_PORT, &gpio_init_struct);

        gpio_init_struct.Pin = ATIM_TIMX2_PWM_CH3_GPIO_PIN;         /* 通道y的GPIO口 */
        HAL_GPIO_Init(ATIM_TIMX2_PWM_CH3_GPIO_PORT, &gpio_init_struct);
        
        HAL_NVIC_SetPriority(ATIM_TIMX2_PWM_IRQn, 2, 3);
        HAL_NVIC_EnableIRQ(ATIM_TIMX2_PWM_IRQn);
    }
}
