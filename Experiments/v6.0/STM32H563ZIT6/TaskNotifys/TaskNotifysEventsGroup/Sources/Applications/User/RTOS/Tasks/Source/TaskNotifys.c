...
#define EVENTS_GROUP_MASK                           0x01
#define EVENTS_GROUP_PUSH_BUTTON_PRESSED            EVENTS_GROUP_MASK << 0
#define EVENTS_GROUP_TIMER_REACHED                  EVENTS_GROUP_MASK << 1
...
#define xTaskNotify( xTaskToNotify, ulValue, eAction ) \
    xTaskGenericNotify( ( xTaskToNotify ), ( tskDEFAULT_INDEX_TO_NOTIFY ), ( ulValue ), ( eAction ), NULL )
            ...
            */
            xTaskNotify(StartNotifyTakeTaskHandle, EVENTS_GROUP_PUSH_BUTTON_PRESSED, eSetBits);
...
void StartTimerTask(void *argument)
{
    ...
    for (;;)
    {
        if (counter ++ == 50)
        {
            xTaskNotify(StartNotifyTakeTaskHandle, EVENTS_GROUP_TIMER_REACHED, eSetBits);
...
void StartNotifyTakeTask(void *argument)
{
    ...
    uint32_t notificationValue, event_bits = 0;
...
#define xTaskNotifyWait( ulBitsToClearOnEntry, ulBitsToClearOnExit, pulNotificationValue, xTicksToWait ) \
    xTaskGenericNotifyWait( tskDEFAULT_INDEX_TO_NOTIFY, ( ulBitsToClearOnEntry ), ( ulBitsToClearOnExit ), ( pulNotificationValue ), ( xTicksToWait ) )
        ...
        */
        ...
        if (notificationValue & EVENTS_GROUP_PUSH_BUTTON_PRESSED)
            event_bits |= EVENTS_GROUP_PUSH_BUTTON_PRESSED;
               
        if (notificationValue & EVENTS_GROUP_TIMER_REACHED)
            event_bits |= EVENTS_GROUP_TIMER_REACHED;
        
        if (event_bits == (EVENTS_GROUP_PUSH_BUTTON_PRESSED | EVENTS_GROUP_TIMER_REACHED))
        {
            printf("The Task Notifys to sumulate Event Group is received.\n\n");
