...
void StartHighPriorityTask(void *argument)
{
    ...
    for (;;)
    {
        printf("High Priority Task is attmpting to acquire the mutex semaphore.\n");
        
        if (pdPASS == xSemaphoreTake(mutexSemaphore, portMAX_DELAY))
        {
            printf("High Priority Task acquired the mutex semaphore.\n");
            printf("High Priority Task delays with HAL_Delay(1000) for 1 seconds.\n");
            
            HAL_Delay(1000);
            
            if (pdPASS == xSemaphoreGive(mutexSemaphore))
            {
                printf("High Priority Task released the mutex semaphore.\n\n");
...
{    
    for (;;)
    {
        printf("Low Priority Task is attmpting to acquire the mutex semaphore.\n");
        
        if (pdPASS == xSemaphoreTake(mutexSemaphore, portMAX_DELAY))
        {
            printf("Low Priority Task acquired the mutex semaphore.\n");
            ...
            if (pdPASS == xSemaphoreGive(mutexSemaphore))
            {
                printf("Low Priority Task released the mutex semaphore.\n\n");
