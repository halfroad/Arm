...
QueueHandle_t binarySemaphoreHandle;
...
void StartGiveReasourceToSemaphoreTask(void *argument)
{
    ...
    for (;;)
    {
        if (GPIO_PIN_SET == HAL_GPIO_ReadPin(PUSH_BUTTON_GPIO_Port, PUSH_BUTTON_Pin)
            && counter ++ >= 10)
        {
            if (pdPASS == xSemaphoreGive(binarySemaphoreHandle))
            {
                printf("Binary semaphore is released. Number of Acquisitions = %d\n\n", ++ numberOfAcquisitions);
...
{    
    for (;;)
    {
        if (pdPASS == xSemaphoreTake(binarySemaphoreHandle, portMAX_DELAY))
        {
            printf("Binary Semaphore is acquired.\n");
        }
    }
