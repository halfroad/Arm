...
void ScheduleTasks(void)
{
    ...
    PeriodicTimerHandle = xTimerCreate( "PeriodicTimerHandleName", /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
                                    500,
                                    pdTRUE,
                                    (void *)1,
...
void OneTimeTimerCallback(TimerHandle_t xTimer)
{
    ...
    printf("The times occured on One Time Timer is %#x.\n", ++ times);
}
...
void PeriodicTimerCallback(TimerHandle_t xTimer)
{
    ...
    if (times == 0x20)
        xTimerStop(NULL, portMAX_DELAY);

    HAL_GPIO_TogglePin(GREEN_LED_GPIO_Port, GREEN_LED_Pin);
    HAL_GPIO_TogglePin(YELLOW_LED_GPIO_Port, YELLOW_LED_Pin);
    HAL_GPIO_TogglePin(RED_LED_GPIO_Port, RED_LED_Pin);

    printf("The times occured on Periodic Timer is %#x.\n", ++ times);
}
