...
#include "./Motors/StepperMotors/Include/StepperMotorController.h"
...
int main(void)
{
    ...
    */
    ...
    while (1)
    {
        ...
        {
            OLED_Clear();
            OLED_ShowString(0, 0, (uint8_t *)"KEY_0_PRESSED", 8, 1);
            OLED_Refresh();            
        }
        else if (state == KEY_1_PRESSED)
        {            
            OLED_Clear();
            OLED_ShowString(0, 0, (uint8_t *)"KEY_1_PRESSED!", 8, 1);
            OLED_Refresh();
        }
        else if (state == KEY_2_PRESSED)
        {
            OLED_Clear();
            OLED_ShowString(0, 0, (uint8_t *)"KEY_2_PRESSED", 8, 1);
            OLED_Refresh();
        }
