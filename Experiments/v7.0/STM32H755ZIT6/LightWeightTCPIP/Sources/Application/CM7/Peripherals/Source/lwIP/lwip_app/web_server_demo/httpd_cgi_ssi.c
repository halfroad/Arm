...
{ 
	...
	//获取ADC的值
	ADCVal = Get_Adc_Average(19,10); //获取ADC1_CH19的电压值


	//转换为电压 ADCVval * 0.05mv(16位ADC)
	ADCVal = (uint32_t)(ADCVal * 0.05);  
...
const char* LEDS_CGI_Handler(int iIndex, int iNumParams, char *pcParam[], char *pcValue[])
{
	...
	if (iIndex != -1)
	{
		...
		for (i=0; i<iNumParams; i++) //检查CGI参数: example GET /leds.cgi?led=2&led=4 */
		{
			...
			{
				if(strcmp(pcValue[i], "LED1ON") ==0)led1s=0;		 //改变LED1状态
				else if(strcmp(pcValue[i],"LED1OFF") == 0)led1s=1; 
				LED1(led1s); //设置LED1
			}
		}
	}
	beep=PCF8574_ReadBit(BEEP_IO);												//读取beep状态
	if(led1s == 0 && beep == 1) 		return "/STM32_LED_ON_BEEP_OFF.shtml";  //LED1开,BEEP关
	else if(led1s == 0 && beep == 0) return "/STM32_LED_ON_BEEP_ON.shtml";		//LED1开,BEEP开
	else if(led1s == 1 && beep == 0) return "/STM32_LED_OFF_BEEP_ON.shtml";		//LED1关,BEEP开
...
const char *BEEP_CGI_Handler(int iIndex,int iNumParams,char *pcParam[],char *pcValue[])
{
	...
	beep=PCF8574_ReadBit(BEEP_IO);												//读取beep状态
	led1s=GPIO_Pin_Get(GPIOB,PIN0);												//读取LED1状态
	if(led1s == 0 && beep == 1) 		return "/STM32_LED_ON_BEEP_OFF.shtml";	//LED1开,BEEP关
	else if(led1s == 0 && beep == 0) return "/STM32_LED_ON_BEEP_ON.shtml";		//LED1开,BEEP开
	else if(led1s == 1 && beep == 0) return "/STM32_LED_OFF_BEEP_ON.shtml";		//LED1关,BEEP开
	else return "/STM32_LED_OFF_BEEP_OFF.shtml";   //LED1关,BEEP关
