...
{
	struct pbuf *ptr;
	ptr=pbuf_alloc(PBUF_TRANSPORT,strlen((char*)tcp_demo_sendbuf),PBUF_ROM); //申请内存,因为tcp_demo_sendbuf是const类型,定义在内部FLASH,所以使用PBUF_ROM参数
																			 //如果tcp_demo_sendbuf是内存里面的数组,则建议使用PBUF_POOL/PBUF_RAM参数.
	if(ptr)
	{
