...
static void HandleUpperHostPayload(void *protocol, uint8_t *payload);
static HAL_StatusTypeDef Report(MotorControlProtocol *protocol, MotorDriveBoardReportTypes motorDriveBoardReportType);

void InitUpperHostCommunications(MotorControlProtocol *motorControlProtocol, PIDTypeDef *PIDType)
{
    InitSerialCommunications(115200, motorControlProtocol, HandleUpperHostPayload);
    
    InitMotorControlProtocol(motorControlProtocol);
    
    ReportCategory(motorControlProtocol, MotorCategoryDirectCurrentMotor);
    ReportState(motorControlProtocol, MotorStateIdle);
    ReconcileInitialPIDs(motorControlProtocol, MotorDriveBoardReportTypePID1, (float *)(&PIDType -> TargetValue), LOCATION_PROPORTIONAL_FACTOR, LOCATION_INTEGRAL_FACTOR, LOCATION_DERIVATIVE_FACTOR);
}
...
void ReconcileInitialPIDs(MotorControlProtocol *motorControlProtocol, MotorDriveBoardReportTypes motorDriveBoardReportType, float *targetValue, float proportionalFactor, float integralFactor, float derivativeFactor)
{    
...
void AcquirePIDs(MotorControlProtocol *motorControlProtocol, MotorDriveBoardReportTypes motorDriveBoardReportType, float *proportionalFactor, float *integralFactor, float *derivativeFactor)
{
...
static void HandleUpperHostPayload(void *protocol, uint8_t *payload)
{
    MotorControlProtocol *motorControlProtocol = (MotorControlProtocol *) protocol;
        ...
        if (receivedBytes[(receivedBytesAddressOffset + MAXIMUM_RECEIVE_BUFFER_LENGTH - 4) % MAXIMUM_RECEIVE_BUFFER_LENGTH] == MOTOR_CONTROLS_PROTOCOL_HEAD)
        {
...
#endif
            {
                if (receivedBytes[(receivedBytesAddressOffset + MAXIMUM_RECEIVE_BUFFER_LENGTH - 4 + 1) % MAXIMUM_RECEIVE_BUFFER_LENGTH] == UpperHostDispatchMotorCommandAcquireAllParameters)
                {
                    ...
                    Report(motorControlProtocol, MotorDriveBoardReportTypeHallSensorEncoderLocation);
...
static HAL_StatusTypeDef Report(MotorControlProtocol *protocol, MotorDriveBoardReportTypes motorDriveBoardReportType)
{
    ...
    switch (motorDriveBoardReportType)
    {
        ...
        case MotorDriveBoardReportTypeHallSensorEncoderLocation:
...
void ReportCategory(MotorControlProtocol *motorControlProtocol, MotorCategories motorCategory)
{
...
void ReportState(MotorControlProtocol *motorControlProtocol, MotorStates motorState)
{
...
void ReportCurrents(MotorControlProtocol *motorControlProtocol, float phaseUCurrent, float phaseVCurrent, float phaseWCurrent)
{
...
void ReportVoltage(MotorControlProtocol *motorControlProtocol, float voltage)
{
...
void ReportPower(MotorControlProtocol *motorControlProtocol, float power)
{
...
void ReportVelocity(MotorControlProtocol *motorControlProtocol, float velocity)
{
...
void ReportTravelDistance(MotorControlProtocol *motorControlProtocol, float travelDistance)
{
...
void ReportTemperatures(MotorControlProtocol *motorControlProtocol, float motorTemperature, float boardTemperature)
{
...
void ReportWave(MotorControlProtocol *motorControlProtocol, uint8_t channel, int16_t wave)
{
