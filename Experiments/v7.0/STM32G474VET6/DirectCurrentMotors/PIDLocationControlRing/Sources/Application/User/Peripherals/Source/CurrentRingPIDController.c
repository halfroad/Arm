...
void TickTimerPeriodElapsedHandler(TIM_HandleTypeDef *htim)
{
    ...
    if (i ++ % PID_SAMPLING_CYCLE == 0)
    {
        ...
        newPulseWidthModulation = ComposePID(&PIDType, motorControlProtocol.hallSensorLocation);
#else
        newPulseWidthModulation = ComposePID(&PIDType, motorControlProtocol.velocity);
#endif
...
void AssignNewTarget(int16_t newTarget)
{
    ...
    if (PIDType.TargetValue <= -MOTOR_PEAK_PULSE_WIDTH_MODULATION)
        PIDType.TargetValue = -MOTOR_PEAK_PULSE_WIDTH_MODULATION;
    else if (PIDType.TargetValue >= MOTOR_PEAK_PULSE_WIDTH_MODULATION)
        PIDType.TargetValue = MOTOR_PEAK_PULSE_WIDTH_MODULATION;   
}
