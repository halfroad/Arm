...
void InitAdvancedTimer(uint16_t prescaler, uint16_t autoReload,uint16_t defaultCompare)
{
    ...
    TIM6_HandleTypeDef TIM6_Handle = { 0 };
    */
    
    TIMER_CLOCK_ENABLE();
    
    TIM6_Handle.Instance = ADVANCE_TIMER_INSTANCE;
    
    /* TIM6_Handle.Init.Prescaler = __HAL_TIM_CALC_PSC(SystemCoreClock, 1000); */
    TIM6_Handle.Init.Prescaler = prescaler;
    TIM6_Handle.Init.CounterMode = TIM_COUNTERMODE_UP;
    TIM6_Handle.Init.Period = autoReload;
    ...
    HAL_TIM_PWM_ConfigChannel(&TIM6_Handle, &TIM_OC_InitType, ADVANCE_TIMER_CHANNEL);
    ...
    */
    ...
    HAL_TIMEx_ConfigBreakDeadTime(&TIM6_Handle, &TIM_BreakDeadTimeConfigType);
}
