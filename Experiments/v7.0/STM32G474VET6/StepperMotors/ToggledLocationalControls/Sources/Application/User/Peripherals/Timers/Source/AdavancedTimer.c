...
#define RCC_TIMER_CLOCK_ENABLE()                                    do                                      \
                                                                    ...
                                                                    while   (0)
#define TIMER_OUTPUT_COMPARE_MODE                                   TIM_OCMODE_PWM1
...
#define TIMER_IRQN                                                  TIM8_UP_IRQn
#define TIMER_IRQ_HANDLER                                           TIM8_UP_IRQHandler
...
void StartPulseWidthModulation(PulseWidthModulationOutputCompareChannels channel)
{
    ...
    switch (channel)
    {
        case PulseWidthModulationOutputCompareChannel0:
            HAL_TIM_PWM_Start(&TIM_HandleType, TIMER_OUTPUT_COMPARE_CHANNLE_0);
            break;
        
        case PulseWidthModulationOutputCompareChannel1:
            HAL_TIM_PWM_Start(&TIM_HandleType, TIMER_OUTPUT_COMPARE_CHANNLE_1);
            break;
...
{
    HAL_TIM_Base_Stop_IT(&TIM_HandleType);
    
    switch (channel)
    {
        case PulseWidthModulationOutputCompareChannel0:
            HAL_TIM_PWM_Stop(&TIM_HandleType, TIMER_OUTPUT_COMPARE_CHANNLE_0);
            break;
        
        case PulseWidthModulationOutputCompareChannel1:
            HAL_TIM_PWM_Stop(&TIM_HandleType, TIMER_OUTPUT_COMPARE_CHANNLE_1);
            break;
...
{
    static uint32_t counter = 0, ccr = 500;
    ...
    switch (htim -> Channel)
    {
        case HAL_TIM_ACTIVE_CHANNEL_1:
            __HAL_TIM_SET_COMPARE(htim, TIMER_OUTPUT_COMPARE_CHANNLE_0, (counter + ccr) % 0xFFFF);
            break;
        
        case HAL_TIM_ACTIVE_CHANNEL_2:
            __HAL_TIM_SET_COMPARE(htim, TIMER_OUTPUT_COMPARE_CHANNLE_1, (counter + ccr) % 0xFFFF);
            break;
