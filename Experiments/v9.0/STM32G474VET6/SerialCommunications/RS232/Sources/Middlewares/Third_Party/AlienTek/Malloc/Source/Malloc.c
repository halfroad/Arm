...
{
    custom_mem_init,                /* 内存初始化 */
    custom_mem_perused,             /* 内存使用率 */
 ...
 * @retval      无
 */
void custom_mem_copy(void *des, void *src, uint32_t n)
{
    uint8_t *xdes = des;
    uint8_t *xsrc = src;
 ...
 * @retval      无
 */
void custom_mem_set(void *s, uint8_t c, uint32_t count)
{
    uint8_t *xs = s;
 ...
 * @retval      无
 */
void custom_mem_init(uint8_t memx)
{
    uint8_t mttsize = sizeof(MT_TYPE);  /* 获取memmap数组的类型长度(uint16_t /uint32_t)*/
    custom_mem_set(mallco_dev.memmap[memx], 0, memtblsize[memx]*mttsize); /* 内存状态表数据清零 */
 ...
 */
uint16_t custom_mem_perused(uint8_t memx)
{
    uint32_t used = 0;
    uint32_t i;
 ...
 */
uint32_t custom_mem_malloc(uint8_t memx, uint32_t size)
{
 ...
 *   @arg       2, 超区域了(失败);
 */
uint8_t custom_mem_free(uint8_t memx, uint32_t offset)
{
    int i;
 ...
 * @retval      无
 */
void custom_free(uint8_t memx, void *ptr)
{
    uint32_t offset;
    ...
    custom_mem_free(memx, offset);  /* 释放内存 */
}
 ...
 * @retval      分配到的内存首地址.
 */
void *custom_malloc(uint8_t memx, uint32_t size)
{
    uint32_t  offset;
    offset = custom_mem_malloc(memx, size);
 ...
 * @retval      新分配到的内存首地址.
 */
void *custom_realloc(uint8_t memx, void *ptr, uint32_t size)
{
    uint32_t  offset;
    offset = custom_mem_malloc(memx, size);
    ...
    {
        custom_mem_copy((void*)((uint32_t)mallco_dev.membase[memx] + offset), ptr, size);  /* 拷贝旧内存内容到新内存 */
        custom_free(memx, ptr);      /* 释放旧内存 */
