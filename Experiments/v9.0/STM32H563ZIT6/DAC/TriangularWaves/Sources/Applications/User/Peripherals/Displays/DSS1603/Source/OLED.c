...
#include "./Peripherals/Communications/InterIntegratedCircuits/Include/InterIntegratedCircuits.h"
...
void OLED_Init(void)
{
    ...
    OLED_WR_Byte(0xAF, OLED_CMD); //--turn on SSD1306 panel
...
void OLED_WR_Byte(uint8_t data, uint8_t mode)
{
    ...
    WriteByte(0b01111000, AcknowledgeRequired);
    
    if (mode)
        WriteByte(0x40, AcknowledgeRequired);
    else
        WriteByte(0x00, AcknowledgeRequired);
    
    WriteByte(data, AcknowledgeRequired);
...
void OLED_Refresh(void)
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        for (n = 0; n < 128; n++)
        {
            WriteByte(OLED_GRAM[n][i], AcknowledgeRequired);
        }
