...
#include "./Interrupts/CM7/Include/SystemInterrupts.h"
...
void InitSystem(uint32_t pll1_prescalerr_m, uint32_t pll1_vco_multiplication_factor_n, uint32_t pll1_division_factor_p, uint32_t pll1_division_factor_q, uint32_t pll1_division_factor_r)
{
    ...
    Set and reset by software.
    When set, this bit indicates that the SRAM2 is allocated by the CPU1. It causes the D2 domain to
    take into account also the CPU1 operation modes, i.e. keeping D2 domain in DRun when CPU1 is in
    CRun.
