...
#include "./Peripherals/CM7/Include/SerialPeripheralInterfaces.h"
#include "./Peripherals/CM7/Include/NFR24L01P.h"

/* 扩展变量 ------------------------------------------------------------------*/
extern SPI_HandleTypeDef SPI_HandleType;
...
#define IRQ_READ HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_5)
...
uint8_t NRF24L01_Write_Reg(uint8_t reg, uint8_t value)
{
    ...
    ;                                   //使能SPI传输
    status = SPIx_ReadWriteByte(&SPI_HandleType, reg); //发送寄存器号
    SPIx_ReadWriteByte(&SPI_HandleType, value);        //写入寄存器的值
...
uint8_t NRF24L01_Read_Reg(uint8_t reg)
{
    ...
    SelectSlave(SlaveSelected);
    ;                                     //使能SPI传输
    SPIx_ReadWriteByte(&SPI_HandleType, reg);            //发送寄存器号
    reg_val = SPIx_ReadWriteByte(&SPI_HandleType, 0XFF); //读取寄存器内容
...
uint8_t NRF24L01_Read_Buf(uint8_t reg, uint8_t *pBuf, uint8_t len)
{
    ...
    SelectSlave(SlaveSelected);;                                   //使能SPI传输
    
    status = SPIx_ReadWriteByte(&SPI_HandleType, reg); //发送寄存器值(位置),并读取状态值
    for (uint8_t_ctr = 0; uint8_t_ctr < len; uint8_t_ctr++)
    {
        pBuf[uint8_t_ctr] = SPIx_ReadWriteByte(&SPI_HandleType, 0XFF); //读出数据
    }
    
    SelectSlave(SlaveNotSelected);;       //关闭SPI传输
...
uint8_t NRF24L01_Write_Buf(uint8_t reg, uint8_t *pBuf, uint8_t len)
{
    ...
    for (uint8_t_ctr = 0; uint8_t_ctr < len; uint8_t_ctr++)
    {
        SPIx_ReadWriteByte(&SPI_HandleType, *pBuf++); //写入数据
    }
...
uint8_t NRF24L01_TxPacket(uint8_t *txbuf)
{
    ...
    SelectSlave(SlaveNotSelected);                                                //启动发送
...
void NRF24L01_RX_Mode(void)
{
    ...
    SelectSlave(SlaveNotSelected); // CE为高,进入接收模式
...
void NRF24L01_TX_Mode(void)
{
    ...
    SelectSlave(SlaveNotSelected);                                            // CE为高,10us后启动发送
