...
#define CHIP_SELECT_LOW								SPI_CHIP_SELECT_LOW(GPIOA, GPIO_Pin_9)
#define CHIP_SELECT_HIGH							SPI_CHIP_SELECT_HIGH(GPIOA, GPIO_Pin_9)

#define W25QXX_JEDEC_DIRECTIVE_READ_BYTE			0x00

#define W25QXX_DIRECTIVE_WAIT_FLAG					0x01

#define W25QXX_DIRECTIVE_WRITE_STATUS_REGISTER		0x01	/* Byte0|byte1|byte2, 0x01|S7 ~ S0|S8 ~ S5, S means Status. */
...
#define W25QXX_DIRECTIVE_FULL_WIPE					0x60	/* Byte0, 0xC7/0x60. */
...
#define W25QXXX_FLASH_PAGES_NUMBER(capacity)		capacity * 1024 * 1024 / 256
...
uint16_t w25qxx_flash_read_identifier(void)
{
	...
	/* Read a byte. */
	identifier0 = spi_write_read(W25QXX_JEDEC_DIRECTIVE_READ_BYTE);
	/* Read a byte. */
	identifier1 = spi_write_read(W25QXX_JEDEC_DIRECTIVE_READ_BYTE);
	/* Read a byte. */
	identifier2 = spi_write_read(W25QXX_JEDEC_DIRECTIVE_READ_BYTE);
...
void w25qxx_flash_wait_until_write_end(void)
{
	...
	spi_write_read(W25QXX_DIRECTIVE_READ_STATUS_REGISTER_1);
...
void w25qxx_flash_page_write(uint32_t address, uint8_t *bytes, uint32_t length)
{
	...
	spi_write_read(W25QXX_DIRECTIVE_PAGE_PROGRAMMING);
