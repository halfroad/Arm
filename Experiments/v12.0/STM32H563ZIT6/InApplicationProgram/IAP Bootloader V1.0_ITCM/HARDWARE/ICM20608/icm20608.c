...
u8 ICM20608_Init(void)
{
    u8 res;
    IIC_Init();//初始化IIC总线
    ICM_Write_Byte(ICM_PWR_MGMT1_REG, 0X80);	//复位ICM20608
    delay_ms(100);
    ICM_Write_Byte(ICM_PWR_MGMT1_REG, 0X00);	//唤醒ICM20608
    ...
    ICM_Write_Byte(ICM_INTBP_CFG_REG, 0X80);	//INT引脚低电平有效
    ICM_Write_Byte(0x1D, 1);					//INT引脚低电平有效
    res = ICM_Read_Byte(ICM_DEVICE_ID_REG);
    if(res == 0XAF) //器件ID正确
    {
        ICM_Write_Byte(ICM_PWR_MGMT1_REG, 0X01);//设置CLKSEL,PLL X轴为参考
        ICM_Write_Byte(ICM_PWR_MGMT2_REG, 0X00);//加速度与陀螺仪都工作
        ICM_Set_Rate(50);						//设置采样率为50Hz
    }
    else return 1;
...
//设置ICM20608陀螺仪传感器满量程范围
//fsr:0,±250dps;1,±500dps;2,±1000dps;3,±2000dps
//返回值:0,设置成功
//    其他,设置失败 
u8 ICM_Set_Gyro_Fsr(u8 fsr)
{
    return ICM_Write_Byte(ICM_GYRO_CFG_REG, fsr << 3); //设置陀螺仪满量程范围
}

//设置ICM20608加速度传感器满量程范围
//fsr:0,±2g;1,±4g;2,±8g;3,±16g
//返回值:0,设置成功
//    其他,设置失败
u8 ICM_Set_Accel_Fsr(u8 fsr)
{
    return ICM_Write_Byte(ICM_ACCEL_CFG_REG, fsr << 3); //设置加速度传感器满量程范围
}
