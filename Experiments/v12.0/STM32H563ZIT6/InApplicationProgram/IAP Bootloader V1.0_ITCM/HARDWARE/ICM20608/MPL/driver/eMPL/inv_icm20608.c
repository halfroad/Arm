...
//返回值:0,正常
//    其他,失败
u8 icm_mpl_get_data(float *pitch,float *roll,float *yaw)
{
    ...
    long data[9];
    int8_t accuracy;
	if(icm_read_fifo(gyro, accel_short, &timestamp, &sensors,&more))return 1;	 
    if(sensors&INV_XYZ_GYRO)
    {
        inv_build_gyro(gyro,timestamp);          		//把新数据发送给MPL
        icm_get_temperature(&temperature,&timestamp);	//读取温度
        inv_build_temp(temperature,timestamp);  	 	//把温度值发给MPL，只有陀螺仪需要温度值
    }
    if(sensors&INV_XYZ_ACCEL)
    {
        ...
        inv_build_accel(accel,0,timestamp);      //把加速度值发给MPL
    }
